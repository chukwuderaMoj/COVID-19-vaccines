...
```{r}
# Write csv file
#write.csv(cdc_covid,"cdc_covid.csv", row.names = FALSE)
```
...
#Movement_data_agg_raw<- Movement_Data_Aggregated_UPDATED

Movement_month_comp_raw <- read.csv(file = "../Movement_Month_Comp_UPDATED.csv")
#Movement_month_comp_raw <- Movement_Month_Comp_UPDATED
```
...
#Policy_Mandate_raw <- Policy_Mandate_Data_UPDATED
```
...
To assess whether the distribution of the errors is homoskedastic, we examined the residuals versus fitted plot. From the plot it looks like the points below our line are more spaced out and the ones below are a bit more dense and "heavy" but not significant. There are few residuals on the left and extreme right that are causing some unevenness but not too significant. This looks nice and balanced for the most part. Therefore, We assume that Homoskedasticity applies for our sample and we are not dealing with Heteroskedastic errors. 
...
We also looked at the qq plot which confirms the same with the distribution almost being close to the normal line but with some deviation towards bottom left and at the extreme right mostly due to noise in data but not significant. Therefore we assume that normality of errors is satisfied and not a concern.
